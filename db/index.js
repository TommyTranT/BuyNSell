...
const getListingWithId = function(id) {
  ...
  return pool
    .query(`
      SELECT listings.*, listings.id as listing_id, users.name as owner_name, users.id as owner_id
      FROM listings
...
const getMessages = function(id) {
  return pool
    .query(`
      SELECT listing_id, users.name as sender_name, listings.title as listing_title, contents, time_sent
      FROM messages
